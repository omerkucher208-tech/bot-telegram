...
def check_user_membership(user_id):
    ...
    try:
        ...
        # ئەگەر ئەندام بێت یان ئەدمین یان خاوەن کەناڵ
        if member.status in ['member', 'administrator', 'creator']:
            return True
    except Exception as e:
        print(f"Error checking membership: {e}")
        # بۆ ئەوەی ئەگەر بۆت ئەدمین نەبێت یان کێشەیەک هەبێت ڕێگری لە بەکاربەری نەکات، 
        # دەتوانیت کاتیڤ بۆ تاقیکردنەوە ڤەکەی (بەڵام سەرەتا بۆت بکە ئەدمین)
        return True  
    return False
...
def send_welcome(message):
    ...
    # پشکنینا جۆینبوونا کەناڵی پێش هەموو شتەکی
...
def callback_handler(call):
    ...
    # پشکنینا جۆینبوونێ بۆ هەموو کلیکان
    ...
    # ژێر بەشێن VIP (تەلەگرام و ئینستگرام)
...
def show_main_menu(chat_id, message_id=None, is_new=False):
    ...
    markup.add(InlineKeyboardButton("🎁 بەشێ فەیک", callback_data="menu_fake"))
    markup.add(InlineKeyboardButton("⭐ ڤەکرنا بەشێ VIP", callback_data="vip"))
    markup.add(InlineKeyboardButton("🌟 تایبەت تورسی", callback_data="special_tursi"))
    markup.add(InlineKeyboardButton("🎟 بکارئینانا کۆدێ دیاری", callback_data="code"))
    markup.add(InlineKeyboardButton("🎁 دیاریا ڕۆژانە (+10 پۆینت)", callback_data="daily_bonus"))
    markup.add(InlineKeyboardButton("🌐 لینکێ ئینڤایتێ (Ref)", callback_data="ref_link"))
...
def callback_handler(call):
    ...
    if call.data == "daily_bonus":
        ...
        text = (
            "🌟 **بەخێرهاتن بۆ بەشی تایبەت تورسی!**\n\n"
            "ئەم بەشە تایبەتە بە خزمەتگوزاری و ئەپدەیتێن نوێ. یەکێک لە بژاردەکانی خوارەوە هەڵبژێرە:"
        )
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("📢 کەناڵێ ڕەسمی", url="https://t.me/TURSE_INFO"))
        markup.add(InlineKeyboardButton("🔙 ڤەگەر بۆ مێنویێ سەرەکی", callback_data="back_home"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
